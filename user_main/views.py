    ...
    if request.user.is_authenticated:
      if request.method == "GET":
        ...
        try:
            ...
            connects_received = Connect.objects.filter(receiver=user)
            context = {
                "name_letters_excluding_the_first": name[1::],
                ...
                "total_connects_received": len(connects_received),
                "connects_received": connects_received,
            }
        except BaseException as e:
          print(e)
          context = {}
...
@csrf_exempt
def get_connection(request, *args, **kwargs):
  data = json.loads(request.body)
  id = data.get("id")
  if id is not None:
    try:
        connect = Connect.objects.get(id=id)
        return JsonResponse({ "connect": connect.to_json_new() })
    except BaseException as e:
...
@csrf_exempt
def accept_connection(request, *args, **kwargs):
    data = json.loads(request.body)
    if data is not None:
        try:
            connect = Connect.objects.get(id=data.get("id"))
            connect.status = "Accepted"
            connect.save()
            return JsonResponse({"data": "Connection accepted"})

        except BaseException as e:
            print(e)
            return JsonResponse({"data": "An error occurred"})
